...
//
// Modified for Chrome Helper App
// Kreg Hanning, January 2015
//
...
(function(ext) {
  ...
  var wedoCommand = new Uint8Array(8); wedoCommand[0] = 0x40;
  function sendMotorState() {
    if (connected) {
      // Each motor is controlled by a signed byte whose sign
      // determines the direction and absolute value the power
      wedoCommand[1] = motorValue(0);
      wedoCommand[2] = motorValue(1);
      //device.write(wedoCommand.buffer);
      sendHelper(wedoCommand.buffer);
    }
  }
  ...
  var startHelperSocket = function() {
    ...
    socket.onmessage = function(event) {
      var data = new Uint8Array(event.data.split(','));
      rawData = data.buffer;
    };
  };
  ...
  ext._getStatus = function() {
    if(!connected) return {status: 1, msg: 'LEGO WeDo disconnected'};
  ...
  ScratchExtensions.register('LEGO WeDo (Chrome)', descriptor, ext);
